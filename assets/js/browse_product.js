...
var basicResultController = (function() {
    ...
    obj.showResult = function(catalogList, cart) {
        ...
        for (i = 0; i < catalogList.length; i++) {
            ...
            catalogHTML += '</div>'
            if (cart.length > 0) {
                var cartItem = cart.filter(function(cartItem) { return cartItem.item_id == catalogList[i]["productId"]; });
                if (cartItem.length > 0) {
                    catalogHTML += '<div class="quantity"><label>quantity </label><input id="inputQuantity_' + catalogList[i]["productId"] + '" style="height:30px; width:100%" onchange="changeQuantity(' + catalogList[i]["productId"] + ',this.value)" min="1" max="100" value="' + cartItem[0].qty + '" type="number"><button class="button" id="btnAddToCart_' + catalogList[i]["productId"] + '" onclick="funcAddToCart(' + catalogList[i]["productId"] + ')" type="button" style="width:100%">added to cart</button></div>'
                } else {
                    catalogHTML += '<div class="quantity"><label>quantity </label><input id="inputQuantity_' + catalogList[i]["productId"] + '" style="height:30px; width:100%" onchange="changeQuantity(' + catalogList[i]["productId"] + ',this.value)" min="1" max="100" value="1" type="number"><button class="button" id="btnAddToCart_' + catalogList[i]["productId"] + '" onclick="funcAddToCart(' + catalogList[i]["productId"] + ')" type="button" style="width:100%">add to cart</button></div>'
                }
            }
