...
async function showAddressSlider(addArr) {
  ...
  if ($('input[name="select"]:checked').val() === "2") {
    var addSliderHTML = "";
    $("#addressSlider").empty(addSliderHTML);
    for (i = 0; i < addArr.length; i++) {
      const address = addArr[i];

      addSliderHTML += '<div class="optionsecoptions">';

      addSliderHTML += "<br>";
      if (address.is_billable != 0 && billing_add_count == 0) {
        // addSliderHTML += '<div class="address-billing" stytle="display: flex; align-items: center; justify-content: center;">'
        // addSliderHTML += '<input id="billingAddrCheckbox" type="radio" checked="checked" class="billingChk" value="'+address.id+'" style="height: 18px; width: 18px;" >'
        // addSliderHTML += ''+ address.address+''
        // addSliderHTML += '</div>'
        // billing_add_count += 1
      } else if (address.is_billable != 0 && billing_add_count > 0) {
        addSliderHTML +=
          '<div class="address-billing" style="display:flex; width:100%" >';
        addSliderHTML +=
          '<input id="billingAddrCheckbox" type="radio"  class="billingChk" value="' +
          address.id +
          '" style="height: 18px; width: 18px; margin-top: 2px" >';
        addSliderHTML +=
          '<p style=" font-size: 15px; margin-left: 10%; width:100%;">' +
          titleCase(str(address.address)) +
          "</p>";
        addSliderHTML += "</div>";
      }
      addSliderHTML += "<br>";
  ...
  } else if ($('input[name="select"]:checked').val() === "3") {
    var onResponse = function (response) {
      ...
      for (j = 0; j < pickupAddrList.length; j++) {
        // const address = addArr[i];
        if (
          pickupAddrList[j]["area_code"]["sa4_name"]
            .toLowerCase()
            .includes("melbourne")
        ) {
          addSliderHTML +=
            '<div class="optionsecoptions" style="border:1px solid #ccc; ; width: initial;;font-size: 14px;width: 100%;">';
          // addSliderHTML += ''+ pickupAddrList[j]["address"]+', '+pickupAddrList[j]["area_code"]["sa4_name"]+', '+pickupAddrList[j]["state"]["name"]+''
          addSliderHTML += pickupAddrList[j]["address"];
          addSliderHTML += "<br>";
          // if(address.is_billable != 0){
          addSliderHTML +=
            '<input id="shippingAddrCheckbox" type="checkbox" checked="checked" class="billingChk" value="' +
            pickupAddrList[j]["id"] +
            '" ><label style="font-size:13px">Pickup Address</label>';
          // break;
          // }
      ...
      for (i = 0; i < addArr.length; i++) {
        ...
        if (address.is_billable != 0 && billing_add_count == 0) {
          ...
          addSliderHTML += "<br>";
          addSliderHTML +=
            '<input id="billingAddrCheckbox" type="checkbox" checked="checked" class="billingChk" value="' +
            address.id +
            '" ><label style="font-size:13px">Billing Address</label>';
          // break;
          billing_add_count += 1;
          addSliderHTML += "</div>";
          addSliderHTML += "<br>";
...
$(document).ready(function () {
  ...
  $("#deliveryOption").change(function () {
    ...
    getQuoteDetails(quoteId, function (response) {
      showCheckoutOrderSummary(
        response.data.quote.quote_line,
        response.data.quote
      );
    });
  });
...
function confirmOrder() {
  const orderNotes =
    $("#inputOrderNotes").val().length > 0 ? $("#inputOrderNotes").val() : null;
  const poNumber =
    $("#inputPoName").val().length > 0 ? $("#inputPoName").val() : null;
  ...
  const sched_delivery_date =
    $("#deliveryType").val() == 3 ? $("#deliveryDateInput").val() : null;
  ...
  createOrder(
    ...
    function (response) {
      ...
      if (response.data.data.payment) {
        $("#btConfirmOrder").hide();
        $("#payment_method").show();
        const partialPayment = safeAccess(
          ["data", "data", "payment", "partial"],
          response,
          0
        );
        const fullPayment = safeAccess(
          ["data", "data", "payment", "full"],
          response,
          0
        );
        const paymentTypeDropdown = $("#inputPaymentType");
        paymentTypeDropdown.html("");
        paymentTypeDropdown.append(
          $("<option>")
        ...
        if (partialPayment != fullPayment) {
          paymentTypeDropdown.append(
            $("<option>")
              .text("Partial Payment $" + partialPayment)
              .val(partialPayment)
          );
        }
        ...
        $("#btProceedToPayment").click(function () {
          ...
          if ($("#inputPaymentMode").val() == 2) {
            ...
            notifyOrderSuccess(
              safeAccess(["data", "data", "order"], response, null)
            );
          }
        });
      } else {
        var orderId = safeAccess(
          ["data", "data", "order", "payment", "order_id"],
          response,
          null
        );
        if (!orderId) {
          orderId = safeAccess(
            ["data", "data", "order", 0, "id"],
            response,
            -1
          );
        }
...
async function showCheckoutOrderSummary(orderSummary, orderDetails) {
  ...
  for (i = 0; i < orderSummary.length; i++) {
    ...
    orderSummaryHTML +=
      "<td> " +
      orderSummary[i]["item"]["name"] +
      " <strong> × " +
      orderSummary[i]["qty"] +
      "</strong></td>";
    orderSummaryHTML +=
      "<td> $" + orderSummary[i]["total"].toLocaleString("en-AU") + "</td>";
    orderSummaryHTML += "</tr>";
  }
  orderSummaryHTML += "</tbody>";
  ...
  orderSummaryHTML +=
    "<td>$" + orderDetails["sub_total"].toLocaleString("en-AU") + "</td>";
  orderSummaryHTML += "</tr>";
  orderSummaryHTML += "<tr>";
  ...
  if ($("#deliveryOption").val() == 2) {
    orderSummaryHTML +=
      "<td><strong>$" +
      orderDetails["shipping_cost"].toLocaleString("en-AU") +
      "</strong></td>";
  } else if ($("#deliveryOption").val() == 3) {
  ...
  orderSummaryHTML +=
    "<td><strong>$" +
    orderDetails["total_tax"].toLocaleString("en-AU") +
    "</strong></td>";
  orderSummaryHTML += "</tr>";
  ...
  if ($("#deliveryOption").val() == 2) {
    orderSummaryHTML +=
      "<td><strong>$" +
      orderDetails["total"].toLocaleString("en-AU") +
      "</strong></td>";
  } else if ($("#deliveryOption").val() == 3) {
    orderSummaryHTML +=
      "<td><strong>$" +
      (orderDetails["total"] - orderDetails["shipping_cost"]).toLocaleString(
        "en-AU"
      ) +
      "</strong></td>";
  }
